...
#define SMB2_FILE_FULL_INFO 7		// anche EA_INFO
...
typedef struct {
	...
	uint16_t lenChallenge;
	uint16_t maxlenChallenge;
	uint32_t ofsChallenge;
	uint8_t reserved[8];
	uint16_t lenInfo;
	uint16_t maxlenInfo;
	uint32_t ofsInfo;
...
public:
	...
	static char *uniDecode(const uint8_t *,int16_t,char *);
	static CTime FiletimeToTime(uint64_t);
	static uint64_t TimeToFiletime(CTime);
	...
	int GetVolumeInfo(char *,CTime *);
	int VolumeInfo(uint64_t *,uint64_t *,uint32_t *);
	int FileStat(LPCTSTR s,struct stat *);
  int SetFileTime(const char *, CTime);
	int Attrib(const char *,uint8_t,uint8_t);
...
private:
	...
	static uint64_t gettime(CTime t=0);
